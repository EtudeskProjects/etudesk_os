...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    if (!req.talentId) {
      return res.status(403).json({ error: 'You must complete your profile to create an organization' });
    }

    // Check if user has verified identity (check kyc_verifications table)
    ...
    if (identityCheck.rows.length === 0) {
      return res.status(403).json({
        ...
        message: 'Vous devez vérifier votre identité avant de créer une organisation. Uploadez une pièce d\'identité dans votre profil.'
      });
    }
...
router.put('/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Check ownership or membership with edit permissions
    ...
    if (ownerCheck.rows.length === 0 && !hasEditPermission) {
      return res.status(403).json({ error: 'You do not have permission to edit this organization' });
    }
