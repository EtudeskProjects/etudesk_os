...
async function main() {
  ...
  await pool.query(
    ...
    [orgId, 'Etudesk Demo Org', 'etudesk-demo-org', ['STARTUP'], ['DIGITAL'], 'Organisation de démonstration', 'Abidjan', 'CI']
  );
  ...
  for (const tt of talents) {
    ...
    await pool.query(
      ...
      [tId, tt.slug, tt.first, tt.last, tt.email, 'Abidjan', 'CI', ['DIGITAL'], tt.tags]
    );
  ...
  for (const op of opps) {
    ...
    await pool.query(
      ...
      [opId, op.title, op.slug, op.contract, op.summary, ['DIGITAL'], JSON.stringify([{ city: 'Abidjan', country: 'CI' }]), organizationId]
    );
  ...
  await pool.query(
    `INSERT INTO communities (id, name, slug, type, description, access_type, visibility, sectors, city, country, status, organization_id, created_by)
     ...
     ON CONFLICT (slug) DO UPDATE SET name = EXCLUDED.name RETURNING id`,
    [commId, 'Dev Community Abidjan', 'demo-dev-community', 'Communauté des développeurs', JSON.stringify(['DIGITAL']), 'Abidjan', 'CI', organizationId]
  );
  ...
  console.log('  community Dev Community Abidjan');
  ...
  await pool.query(
    `INSERT INTO spaces (id, name, slug, type, surface_m2, capacity, description, city, country, sectors, equipment, is_bookable, visibility, status, organization_id)
     VALUES ($1,$2,$3,'WORKSHOP',80,20,$4,'Abidjan','CI',$5,$6,true,'PUBLIC','ACTIVE',$7)
     ON CONFLICT (slug) DO UPDATE SET name = EXCLUDED.name RETURNING id`,
