...
export async function notifyInterviewScheduled(
  ...
  try {
    ...
    const notificationId = await createNotification({
      ...
      referenceType: 'application',
      referenceId: applicationId,
      data: { interviewDate, interviewType, interviewLocation, opportunityTitle: app.opportunity_title },
    });

    if (prefs.email_interview_reminder) {
    ...
    if (prefs.push_interview_reminder) {
      await sendPushNotification(app.talent_id, title, body, {
        type: 'INTERVIEW_SCHEDULED',
        applicationId,
        interviewDate,
