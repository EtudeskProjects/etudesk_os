...
export function getLanguageInstructions(language?: PromptLanguage, _country?: string) {
  ...
  const typographyRule = isFrench
    ? 'TYPOGRAPHIE (RÈGLE ABSOLUE): écris toujours un français naturel avec les accents et diacritiques corrects: compétences, opportunités, précision, développement, départ, généré, à, où, ça. N’écris jamais du français ASCII sans accents. N’utilise JAMAIS le tiret cadratin "—" ni le demi-cadratin "–". Utilise toujours un trait d’union simple "-", une virgule, des parenthèses, deux points, ou deux phrases. Cette règle vaut pour TOUT: texte, listes, titres, labels de graphiques, documents générés.'
    : '**Typography (ABSOLUTE)**: NEVER use the em-dash "—" or en-dash "–". Use a simple hyphen "-", a comma, parentheses, a colon, or split into two sentences instead. This applies to every word you write: prose, lists, titles, chart labels, summaries, confirmations, generated documents.';
...
French MUST include correct accents and diacritics. Never write ASCII-only French.
...
${typographyRule}`,
      ...
      finalReminder: 'Respond in FRENCH with correct accents and diacritics. Every word. No exceptions. ' + typographyRule,
