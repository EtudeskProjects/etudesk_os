...
import { opportunityService, communityService, hubService, bookmarkService } from '../../src/services';
...
const PLACEHOLDER_IMAGES = {
  ...
  hub: 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=200&q=80',
...
export default function ExploreScreen() {
    ...
    try {
      ...
      if (response.data) {
        setBookmarkedItems({
          ...
          hubs: new Set(response.data.hubs || []),
  ...
  const CATEGORIES_TRANSLATED = [
    ...
    { id: 'hubs' as Category, label: t('explore.categories.hubs'), icon: MapPin, image: 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=400&q=80' },
  ...
  const allFilteredHubs = useMemo(() => {
    return sortItems(filterBySearch(hubs, searchQuery), sortOption);
  }, [hubs, searchQuery, sortOption]);
  ...
  const filteredHubs = useMemo(() => {
    return allFilteredHubs.slice(0, displayedCounts.hubs);
  }, [allFilteredHubs, displayedCounts.hubs]);
  ...
  const hasMore = useMemo(() => ({
    ...
    hubs: allFilteredHubs.length > displayedCounts.hubs,
    communities: allFilteredCommunities.length > displayedCounts.communities,
  }), [allFilteredOpportunities.length, allFilteredHubs.length, allFilteredCommunities.length, displayedCounts]);
  ...
  const toggleBookmark = async (id: string, category: Category) => {
    const categoryKey = category === 'opportunities' ? 'opportunities' :
                        category === 'hubs' ? 'hubs' : 'communities';
          ...
          >
            ...
            >
              <View style={[
                ...
                isActive && styles.categoryTabOverlayActive
              ]}>
                <IconComponent
                  size={ICON.size.md}
                  color="#FFFFFF"
  ...
  const renderHubItem = ({ item, index }: { item: Hub; index: number }) => {
    return (
      <HubCard
        hub={item}
        onPress={() => navigateToDetail('hub', item.id)}
        isLast={index === filteredHubs.length - 1}
        showBookmark={true}
        isBookmarked={bookmarkedItems.hubs.has(item.id)}
        onBookmarkToggle={() => toggleBookmark(item.id, 'hubs')}
      />
    );
  };
...
const styles = StyleSheet.create({
  ...
  categoryTabOverlayActive: {
    backgroundColor: 'rgba(38, 68, 159, 0.8)',
  },
