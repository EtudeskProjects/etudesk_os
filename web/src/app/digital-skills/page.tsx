...
const copy = {
  fr: {
    title: 'Observatoire des compétences', subtitle: 'Le référentiel vivant du digital',
    search: 'Rechercher une compétence...', colorBy: 'Couleur par', family: 'Famille', type: 'Type',
    families: 'Familles', types: 'Types', all: 'Tout',
    overview: 'Vue d\'ensemble', mapTitle: 'Une carte vivante des compétences.',
    mapDesc: 'Clique un nœud, recherche une compétence ou filtre par famille et par type pour visualiser ses relations - prérequis, voisins et co-occurrences.',
    hubs: 'Compétences les plus connectées', relations: 'relations',
    pre: 'Prérequis', leads: 'Mène vers', sib: 'Voisines', rel: 'Souvent associées',
    train: 'Me former', trainHint: 'La formation se fait dans l\'app Etudesk.', back: 'Vue d\'ensemble',
    footer: 'Clique un nœud, recherche une compétence ou filtre par famille et par type.',
  },
  en: {
    title: 'Skills Observatory', subtitle: 'The living digital referential',
    search: 'Search a skill...', colorBy: 'Colour by', family: 'Family', type: 'Type',
    families: 'Families', types: 'Types', all: 'All',
    overview: 'Overview', mapTitle: 'A living map of skills.',
    mapDesc: 'Click a node, search a skill or filter by family and type to visualise its relations - prerequisites, neighbours and co-occurrences.',
    hubs: 'Most connected skills', relations: 'relations',
    pre: 'Prerequisites', leads: 'Leads to', sib: 'Neighbours', rel: 'Often paired',
    train: 'Train me', trainHint: 'Training happens in the Etudesk app.', back: 'Overview',
    footer: 'Click a node, search a skill or filter by family and type.',
  },
};

const HUBS = SKILLS_BY_DEGREE.slice(0, 16);
  ...
  return (
    <main className="obs">
              ...
              <div className="list">
                {FAMILIES_SORTED.map((f) => (
                  <button key={f.key} className={`row ${family === f.key ? 'active' : ''} ${family && family !== f.key ? 'dim' : ''}`}
                    ...
                    <span className="ric"><f.Icon size={16} strokeWidth={1.5} color={FAMILY_HEX[f.key][isDark ? 'dark' : 'light']} /></span>
              ...
              <div className="list">
                {TYPES_SORTED.map((ty) => (
                  <button key={ty.key} className={`row ${type === ty.key ? 'active' : ''} ${type && type !== ty.key ? 'dim' : ''}`}
                    ...
                    <span className="ric"><ty.Icon size={16} strokeWidth={1.5} color={TYPE_HEX[ty.key][isDark ? 'dark' : 'light']} /></span>
        ...
        <aside className="right">
          {sel && relations ? (
            <div className="detail">
              ...
              <span className="d-meta">
                <span className="type-chip" style={{ color: `var(${TYPE_MAP[sel.type].colorVar})`, background: `var(${TYPE_MAP[sel.type].bgVar})` }}>
                  {(() => { const I = TYPE_MAP[sel.type].Icon; return <I size={12} strokeWidth={1.5} />; })()}{lang === 'fr' ? TYPE_MAP[sel.type].fr : TYPE_MAP[sel.type].en}
                </span>
            ...
            <div className="overview">
              ...
              <p className="o-desc">{t.mapDesc}</p>
              <span className="hub-h">{t.hubs}</span>
              <div className="hubs">
                {HUBS.map((slug) => {
      ...
      <style jsx>{`
        ...
        .d-title { font-size: var(--font-size-xxl); font-weight: var(--font-weight-bold); color: var(--text-primary); margin: 0.5rem 0 1.25rem; line-height: 1.15; }
        ...
        .relg-bar { width: 0.9rem; height: 0.2rem; border-radius: 2px; flex: none; }
