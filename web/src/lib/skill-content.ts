...
export function getSkillContent(slug: string, lang: Lang): SkillContent | null {
  ...
  if (lang === 'fr') {
    ...
    if (!pre.length && !leads.length && !sib.length && !rel.length) {
      paragraphs.push(`${name} s'apprend pas à pas avec ton tuteur IA : quiz, flashcards, exercices résolus et explications multimodales, sans abonnement et avec 30 crédits offerts pour démarrer.`);
    }
  } else {
    ...
    if (!pre.length && !leads.length && !sib.length && !rel.length) {
      paragraphs.push(`${name} is learned step by step with your AI tutor: quizzes, flashcards, solved exercises and multimodal explanations, no subscription and 30 free credits to start.`);
    }
  }
  ...
  const metaDescription = lang === 'fr'
    ? `${name} : ${typeLabel.toLowerCase()} de la famille ${familyLabel}. Apprends-la avec le tuteur IA Etudesk${leads.length ? ` et débloque ${readableList(N(leads), lang, 3)}` : ''}. Sans abonnement, 30 crédits offerts.`.slice(0, 158)
    : `${name}: ${typeLabel.toLowerCase()} in the ${familyLabel} family. Learn it with the Etudesk AI tutor${leads.length ? ` and unlock ${readableList(N(leads), lang, 3)}` : ''}. No subscription, 30 free credits.`.slice(0, 158);
