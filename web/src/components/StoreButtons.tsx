...
export default function StoreButtons({ variant = 'primary', size = 'md', full = false }: Props) {
  ...
  return (
    <div className={`store-buttons ${full ? 'full' : ''}`}>
      <DownloadSoonLink platform="ios" className={`store-btn ${variant} ${size}`}>
        <AppleIcon size={icon} />
        <span className="t"><span className="s">{t.on}</span><span className="n">App Store</span></span>
      </DownloadSoonLink>
      <DownloadSoonLink platform="android" className={`store-btn ${variant} ${size}`}>
        <PlayIcon size={icon} />
      ...
      <style jsx>{`
        ...
        .store-buttons.full :global(.store-btn) { flex: 1; justify-content: center; }
        :global(.store-btn) {
        ...
        :global(.store-btn.lg) { padding: 0.85rem 1.6rem; }
        :global(.store-btn.primary) { background: var(--primary); color: var(--text-on-primary); }
        :global(.store-btn.primary:hover) { opacity: 0.9; color: var(--text-on-primary); text-decoration: none; }
        :global(.store-btn.light) { background: #FFFFFF; color: #18181B; }
        :global(.store-btn.light:hover) { background: #F4F4F5; color: #18181B; text-decoration: none; }
        :global(.store-btn.brand) { background: #000000; color: #FFFFFF; border: 1px solid rgba(255,255,255,0.18); }
        :global(.store-btn.brand:hover) { background: #1A1A1A; color: #FFFFFF; text-decoration: none; }
        :global(.store-btn .t) { display: flex; flex-direction: column; align-items: flex-start; line-height: 1.15; }
        :global(.store-btn .s) { font-size: var(--font-size-xxs); opacity: 0.7; }
        :global(.store-btn .n) { font-size: var(--font-size-sm); font-weight: var(--font-weight-bold); }
        @media (max-width: 768px) {
          .store-buttons { width: 100%; max-width: 320px; }
          :global(.store-btn) { flex: 1; justify-content: center; }
        }
      `}</style>
    </div>
  );
