# Plateformes Digitales

> Centraliser tous les acces sur hello@etudesk.org (compte entreprise)
> Derniere mise a jour : 16 mars 2026

---

## Gouvernement & Regulateurs

| Plateforme | URL | Acces |
|------------|-----|-------|
| e-impots (DGI) | https://e-impots.gouv.ci | [ ] |
| e-CNPS | https://e-cnps.ci | [ ] |
| FNE | https://www.fne.dgi.gouv.ci | [ ] |

## IA & API

| Plateforme | URL | Usage | Acces |
|------------|-----|-------|-------|
| Provider IA generique | Configure via `AI_BASE_URL` | Modeles open-source: LLM, vision, embeddings, image, STT, TTS | [ ] |
| OpenAI | https://platform.openai.com | Moderation gratuite uniquement si activee | [ ] |
| YouTube Data API | https://console.cloud.google.com | Recherche videos (copilot Study) | [ ] |

## Infrastructure & Hebergement

| Plateforme | URL | Usage | Acces |
|------------|-----|-------|-------|
| GitHub | https://github.com/EtudeskProjects/etudesk_os | Repo code (org: EtudeskProjects) | [ ] |
| LWS | https://www.lws.fr | VPS production (backend + DB) | [ ] |
| Cloudflare | https://dash.cloudflare.com | DNS + CDN + SSL | [ ] |
| Resend | https://resend.com | Emails transactionnels (OTP, invitations) | [ ] |
| Paystack | https://dashboard.paystack.com | Paiements FCFA | [ ] |

## Productivite & Communication

| Plateforme | URL | Usage | Acces |
|------------|-----|-------|-------|
| Google Workspace | https://workspace.google.com | 5 emails @etudesk.com | [ ] |

## Domaines & DNS

| Plateforme | URL | Usage | Acces |
|------------|-----|-------|-------|
| Vename | https://www.vename.com | Domaine etudesk.com | [ ] |

## App Stores

### Google Play Console

| Champ | Valeur |
|-------|--------|
| URL | https://play.google.com/console |
| Developer Account ID | 5706324467393602295 |
| Account Type | Organization |
| Developer Name | ETUDESK |
| Email compte | hello@etudesk.org |
| Adresse | Abidjan Cocody Ii Plateaux Residences Aghien, Bloc A Villa 8, Abidjan, Cote d'Ivoire (CI) |
| Website | https://etudesk.com/ |
| Statut | **ACTIF** |

### Apple Developer Program

| Champ | Valeur |
|-------|--------|
| URL | https://developer.apple.com |
| Team | ETUDESK (82VPZK32J5) |
| D-U-N-S | 851758402 |
| Email | hello@etudesk.org |
| Legal Entity | ETUDESK |
| Adresse | Abidjan Cocody Ii Plateaux Residences Aghien, Bloc A Villa 8, Abidjan, Cote d'Ivoire |
| Statut | **ACTIF** — Agreement signe le 02/04/2026 |

### Expo

| Champ | Valeur |
|-------|--------|
| URL | https://expo.dev |
| Usage | Build & deploy mobile (React Native) |
| Acces | [ ] |

## Presence en Ligne

| Plateforme | URL | Acces |
|------------|-----|-------|
| Site web | https://etudesk.com | [ ] |
| Facebook | https://www.facebook.com/etudesk | [ ] |
| YouTube | https://www.youtube.com/@etudesk | [ ] |
| X (Twitter) | https://x.com/etudesk | [ ] |
| LinkedIn | https://www.linkedin.com/company/etudesk | [ ] |
| TikTok | https://www.tiktok.com/@etudesk | [ ] |
| Instagram | https://www.instagram.com/etudesk | [ ] |
